...
async function inspectColumns() {
  console.log("Inspecting columns of student_academic_performance...");
  const res1 = await supabase.from('student_academic_performance').insert({}).select();
  console.log("student_academic_performance insert error or data:", JSON.stringify(res1, null, 2));

  console.log("\nInspecting columns of subject_attendance...");
  const res2 = await supabase.from('subject_attendance').insert({}).select();
  console.log("subject_attendance insert error or data:", JSON.stringify(res2, null, 2));
}
